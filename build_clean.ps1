# ToolTip_Browser Integration Build Script
# This script builds the integrated system step by step

Write-Host "TOOLTIP: Starting Integration Build..." -ForegroundColor Green
...
Write-Host "TOOLTIP: Configuring CMake..." -ForegroundColor Yellow
cmake .. -G "Visual Studio 17 2022" -A x64 -DCMAKE_BUILD_TYPE=Release
if ($LASTEXITCODE -ne 0) {
    Write-Host "CMake configuration failed, trying alternative..." -ForegroundColor Red
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "Build failed, trying single-threaded build..." -ForegroundColor Red
...
if (Test-Path "Release\minimal_test.exe") {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "  minimal_test.exe failed (exit code: $LASTEXITCODE)" -ForegroundColor Yellow
    }
} else {
    Write-Host "  minimal_test.exe not found" -ForegroundColor Yellow
}
...
if (Test-Path "Release\simple_test.exe") {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "  simple_test.exe failed (exit code: $LASTEXITCODE)" -ForegroundColor Yellow
    }
} else {
    Write-Host "  simple_test.exe not found" -ForegroundColor Yellow
}
...
if (Test-Path "Release\web_automation_demo.exe") {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "  web_automation_demo.exe failed (exit code: $LASTEXITCODE)" -ForegroundColor Yellow
    }
} else {
    Write-Host "  web_automation_demo.exe not found" -ForegroundColor Yellow
}
...
if (Test-Path "Release\tooltip_automation_demo.exe") {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "  tooltip_automation_demo.exe failed (exit code: $LASTEXITCODE)" -ForegroundColor Yellow
    }
} else {
    Write-Host "  tooltip_automation_demo.exe not found" -ForegroundColor Yellow
}
...
if (Test-Path "Release\full_integration_demo.exe") {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "  full_integration_demo.exe failed (exit code: $LASTEXITCODE)" -ForegroundColor Yellow
    }
} else {
    Write-Host "  full_integration_demo.exe not found" -ForegroundColor Yellow
}
...
# Final summary
Write-Host ""
Write-Host "SUCCESS: ToolTip_Browser Integration Complete!" -ForegroundColor Green
