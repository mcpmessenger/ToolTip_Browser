@echo off
echo 🚀 ChromiumFresh + NaviGrab Simple Build (No SQLite)
echo ===================================================
...
echo.
echo 🛠️ Building project (simplified version)...
"C:\Program Files\CMake\bin\cmake.exe" -B build -S . -G "Visual Studio 17 2022" -A x64 -DCMAKE_BUILD_TYPE=Release -DUSE_SQLITE=OFF
if %ERRORLEVEL% neq 0 (
    echo ❌ CMake configuration failed
    echo.
    echo 🔧 Let's try building just NaviGrab examples...
    echo.
    cd NaviGrab
    "C:\Program Files\CMake\bin\cmake.exe" -B build -S . -G "Visual Studio 17 2022" -A x64 -DCMAKE_BUILD_TYPE=Release
    if %ERRORLEVEL% neq 0 (
        echo ❌ NaviGrab CMake configuration failed
        exit /b 1
    )
    echo ✅ NaviGrab CMake configuration successful
    echo.
    echo 🔨 Building NaviGrab examples...
    ...
    echo.
    echo 📊 Checking NaviGrab build results...
    if exist "build\bin\Release" (
        echo ✅ NaviGrab build directory found
        echo.
        echo 📄 Built executables:
        dir build\bin\Release\*.exe
    ) else (
        echo ❌ NaviGrab build directory not found
    )
    cd ..
    exit /b 0
)
